...
def select_attachments():
    """
    Display a list of available files and allow the user to select them as attachments.
    ...
    # Use Streamlit to display the list of available files and get the user's selections.
    selected_files = st.multiselect("Select files for attachment:", available_files)
...
def parse_parts(service, parts, folder_name, message):
    ...
    if parts:
        for part in parts:
            ...
            #                     f.write(urlsafe_b64decode(data))
            #                 st.write(f"Attachment saved as: {filename}")
            #                 st.download_button("Download Attachment", data=urlsafe_b64decode(data), file_name=filename, mime=mimeType)
